drop function if exists public.lock_seats(uuid, uuid[]);

create function public.lock_seats(_event_id uuid, _seat_ids uuid[])
...
revoke all on function public.lock_seats(uuid, uuid[]) from public;
grant execute on function public.lock_seats(uuid, uuid[]) to authenticated, service_role;